...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_USER + 1:
	{
		MessageBox(hWnd, _TEXT("Получено сообщение от окна № 1"), _TEXT("Принял"), MB_OK);
		return DefWindowProc(hWnd, message, wParam, lParam);
	}
	case WM_USER + 2:
	{
		MessageBox(hWnd, _TEXT("Получено сообщение от окна № 1 на выключение"), _TEXT("Принял"), MB_OK);
